...
class GetMonthlyAttendanceCommand:
    ...
    def getWorksheetName(week: str, isMorn: bool) -> str:
        ...
        worksheetName = (
            ...
            + "!"
            + (
                DATE_CELL_RANGE_ATTENDANCE_MORN
                if isMorn
                else DATE_CELL_RANGE_ATTENDANCE_NOON
            )
        )
    ...
    def getMonthlyAttendance() -> dict:
        ...
        for week in weeksInPrevMonth:
            rawMonthlyAttendance.update(
                GetMonthlyAttendanceCommand.getWeeklyAttendance(week)
            )
        monthlyAttendance = GetMonthlyAttendanceCommand.filterRawMontlyAttendance(
            rawMonthlyAttendance
        )
    ...
    def execute(update: Update, context: CallbackContext) -> None:
        ...
        update.message.reply_text(
            f"Attendance for {utils.getPrevMonthStr()} is created on the Google drive!"
        )
